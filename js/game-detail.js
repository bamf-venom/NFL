...
function renderGameDetail() {
  ...
  // Check if 24h before game for edit
  const gameDate = new Date(game.game_date);
  const deadline = new Date(gameDate.getTime() - 24 * 60 * 60 * 1000);
  const canEditBet = myBetData && game.status === 'scheduled' && new Date() < deadline;
      ...
      <div class="card my-bet-card animate-fade-in" style="margin-top: 24px; animation-delay: 0.1s;">
        ...
        ${canEditBet ? `
          <p style="font-size: 12px; color: var(--muted); text-align: center; margin-top: 16px;">
            Bearbeitbar bis: ${deadline.toLocaleString('de-DE')}
          </p>
        ` : ''}
